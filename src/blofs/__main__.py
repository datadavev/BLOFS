...
@main.command("index")
@click.argument(
    "source", type=click.Path(path_type=pathlib.Path, exists=True, file_okay=True)
)
@click.argument("metadata", type=click.Path(path_type=pathlib.Path, file_okay=True))
@click.argument("dest", type=click.Path(path_type=pathlib.Path, exists=False))
def index_container(
    source: pathlib.Path, metadata: pathlib.Path, dest: pathlib.Path
) -> None:
...
@click.argument("manifest", type=click.Path(path_type=pathlib.Path, file_okay=True))
...
def mount_container(source, manifest, mountpoint, foreground) -> None:
    """Mount a TAR container as a local file system."""
    fs = blofs.tarfs.TarManifestFileSystem(tar_path=source, manifest_path=manifest)
    fuse.FUSE(
        blofs.tarfs.FSSpecFUSE(fs), str(mountpoint), foreground=foreground, ro=True
    )
