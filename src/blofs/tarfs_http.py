...
def create_app(pid: str, fs, geodata) -> fastapi.FastAPI:
    ...
    app.state.config = {pid: {"fs": fs, "geo": geodata}, "templates": templates}

    @app.get("/{pid}")
    async def read_manifest(request: fastapi.Request, pid):
        context = {"request": request, "pid": pid}
        return app.state.config["templates"].TemplateResponse(
            request, name="imgview.html", context=context
        )

    @app.get("/{pid}/geojson")
    async def geojson_viewer(request: fastapi.Request, pid):
        try:
            return request.app.state.config[pid]["geo"]
        ...
        try:
            ...
            def iterfile():
                ...
                while chunk := file_object.read(chunk_size):
                    yield chunk
                # Ensure the fsspec file object is properly closed
